...
void printxo(int cordXO [])
{
    int i=0,j=0;
    Nokia5110_PrintBMP(0,48, BG, 0);
    Nokia5110_PrintBMP(row1o,col1o, o, 0);
    Nokia5110_PrintBMP(row1x,col1x, x, 0);
    Nokia5110_DisplayBuffer();
    for(i=0; i<3; i++) //for loop for 9 squares in two diemension location locations
    {
        for(j=0; j<3; j++)
        {
            cordNokia(i,j,cordXO);//TO KNOW PLACES OF XO GAME TO PRINT IT INN NOKIA//0,16
            if(location[i][j]=='x') //locations two diemnsion array
            {
                Nokia5110_PrintBMP(cordXO[0],cordXO[1], x, 0);
                Nokia5110_DisplayBuffer();
            }
            if(location[i][j]=='o')
            {
                Nokia5110_PrintBMP(cordXO[0],cordXO[1], o, 0);
                Nokia5110_DisplayBuffer();
            }
        }
    }
...
void Timer2A_Handler(void)
{
    ...
    while(1)
    {
        ...
        if(win=='x')
        {
					  turn=2;
            Nokia5110_ClearBuffer();
            Nokia5110_PrintBMP(28,40, x, 0);  // player ship middle bottom
            Nokia5110_DisplayBuffer();
            Nokia5110_SetCursor(1,1 );
            Nokia5110_OutString("Winner is ");
        }
        if(win=='E')
        {
					  turn=2;
